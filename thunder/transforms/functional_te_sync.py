...
from thunder.executors.functional_teex import _te_fp8_syncronization
...
class TESynchronizationTransform(Transform):
    ...
    def transform_trace_post_optimization(self, computation_trace, **kwargs):
        ...
        for bsym in computation_trace.bound_symbols:
            ...
            elif bsym.sym.name == "te_fp8_state":
            ...
            if bsym.sym.name == "te_functional_fwd":
                last_fp8_linear_idx = len(new_bsyms)
                self.handling_forward_trace = True
            elif bsym.sym.name == "te_functional_bwd":
                last_fp8_linear_idx = len(new_bsyms)
        ...
        with tracectx(new_trace):
            _te_fp8_syncronization(self.fp8_recipe.output, *sync_states, forward=self.handling_forward_trace)
