...
def one_hot_sample_generator(op, device, dtype, requires_grad, **kwargs):
    # Iterate over the samples generated by scatter_add_sample_generator
    for sample_input in scatter_add_sample_generator(op, device, torch.long, False, **kwargs):
        a = sample_input.args[0]
        b = sample_input.kwargs['src'].shape[-1]
        yield SampleInput(a, num_classes=b)
