...
B = torch.randn(2, 32, 64, device=device, dtype=torch.bfloat16)  # 4 matrices of size 5x6
...
result_3d = torch._grouped_mm(A, B, offs=C)
...
def test_2d_mm(a, b, c):
    return torch._grouped_mm(a, b, c)

result_3d = test_2d_mm(A, B, C)
print(f"Result shape from test_2d_mm: {result_3d.shape}")
another_result = thunder.jit(test_2d_mm)(A, B, C)
