...
register_function(torch.Tensor.copy_, copy_)
register_method("copy_", copy_)
...
def abs_(a: NumberLike | TensorLike, /) -> Number | TensorLike:
    return copy_(a, abs(a))
    # return prims.copy_(abs(a), a, grad_enabled=True)
...
def exponential_(a: Tensor, rate: float = 1, *, generator: None | torch.Generator = None) -> Tensor:
    return copy_(a, exponential(a, rate=rate, generator=generator))
...
def add_(
    ...
    *,
    ...
    return copy_(a, add(a, b, alpha=alpha))
...
def bitwise_and_(a, b, /):
    return copy_(a, bitwise_and(a, b))
...
def bitwise_xor_(a, b, /):
    return copy_(a, bitwise_xor(a, b))
...
def floor_divide_(a, b, /):
    return copy_(a, floor_divide(a, b))
...
def logical_and_(a, b, /):
    return copy_(a, logical_and(a, b))
...
def sub_(a, b, /, *, alpha: NumberLike | TensorLike = 1):
    return copy_(a, sub(a, b, alpha=alpha))
...
def true_divide_(a: TensorLike, b: NumberLike | TensorLike, /) -> TensorLike:
    return copy_(a, true_divide(a, b))
...
def addcmul_(a: TensorLike, b: TensorLike, c: TensorLike, /, *, value: None | Number = None) -> TensorLike:
    return copy_(a, addcmul(a, b, c, value=value))
...
def addcdiv_(a: TensorLike, b: TensorLike, c: TensorLike, /, *, value: None | Number = None) -> TensorLike:
    return copy_(a, addcdiv(a, b, c, value=value))
...
def lerp_(start: TensorLike, end: TensorLike, weight: Number | TensorLike) -> TensorLike:
    return copy_(start, lerp(start, end, weight))
...
def clamp_(
    ...
    return copy_(a, clamp(a, min, max))
...
def masked_fill_(a: TensorLike, /, mask: TensorLike, value: NumberLike | TensorLike) -> TensorLike:
    return copy_(a, masked_fill(a, mask, value))
...
def tril_(a: TensorLike, /, diagonal: int = 0, *, fill_value: None | Number = None) -> TensorLike:
    return copy_(a, tril(a, diagonal, fill_value=fill_value))
...
def cumsum_(a: TensorLike, dim: int, *, dtype: None | dtypeLike = None) -> TensorLike:
    return copy_(a, cumsum(a, dim, dtype=dtype))
...
def index_add_(a: TensorLike, /, dim: int, index: TensorLike, source: TensorLike) -> TensorLike:
    return copy_(a, index_add(a, dim, index, source))
...
def index_copy_(a: TensorLike, /, dim: int, index: TensorLike, source: TensorLike) -> TensorLike:
    return copy_(a, index_copy(a, dim, index, source))
...
def scatter_(
    ...
    *,
    ...
    return copy_(a, clang.scatter(a, index, src, dim))
...
def scatter_add_(a: TensorLike, /, dim: int, index: TensorLike, src: TensorLike) -> TensorLike:
    return copy_(a, scatter_add(a, dim, index, src))
...
def index_put_(
    ...
    return copy_(a, index_put(a, indices, values, accumulate))
...
def _native_batch_norm(
    ...
    if training:
        ...
        if running_mean is not None:
            ...
            copy_(running_mean, new_running_mean)
        if running_var is not None:
            ...
            copy_(running_var, new_running_var)
    else:
...
if torch.distributed.is_available():
    ...
    def all_gather_(
        ...
        if async_op:
            ...
            out = out_or_work
        return copy_(output_tensor, out.view(output_tensor.shape))
    ...
    def reduce_scatter_(
        ...
        if async_op:
            ...
            out = out_or_work
        return copy_(output, out.view(output.shape))
