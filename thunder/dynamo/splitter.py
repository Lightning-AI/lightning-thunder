...
def _splitter(
    ...
    if any(p is not None for p in split_gm.parameters(recurse=True)) or any(
        b is not None for b in split_gm.buffers(recurse=True)
    ):
            ...
            for n in graph_module.graph.nodes:
                if n.op == "output":
                    for n in n.all_input_nodes:
                        ...
                        if (
                            "example_value" not in n.meta
                            or not isinstance(n.meta["example_value"], torch.Tensor)
                            or n.meta["example_value"].grad_fn is None
                        ):
