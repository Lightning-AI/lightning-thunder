    ...
    def _run_with_cache_info():
        ...
        with thunder.core.trace.tracectx(TraceCtx()):
            try:
                ...
                print(f"ERROR ++++++++++++++++{node.format_node()}, {e}")
                return False, SplitReason(
                    SplitReasonType.EXCEPTION_META_THUNDER_OP,
                    f"Failed while running meta for node with name: {node.name} and target: {node.target}, see exception field {str(e)}",
                    exception=str(e),
                )
...
class KVCacheManager:
    ...
    def update_cache_initializers(self, attn_nodes: list[torch.fx.Node]):
        ...
        for idx, attn_node in enumerate(attn_nodes):
            ...
            for k, get_cache in self.attn_descriptor.get_cache_initializers(attn_node, self.cache_config).items():
                k_indexed = f"{k}_{idx}"
                self.cm.add_cache(k_indexed, get_cache)
                # cache_in_nodes.append(add_graph_input(gm, k_indexed))

            # store global buffer initializers
            # NOTE: we have to check against existing keys to make sure nothing is registered twice...
            for k, get_buffer in self.attn_descriptor.get_global_buffer_initializers(attn_node).items():
    ...
    def transform_graph(self, gm: torch.fx.GraphModule, cm, cached_attn_args_names, source_op):
        ...
        for idx, attn_node in enumerate(src_attn_nodes):
            # pick out GEMMs
            qkv = attn_node.args[: self.attn_descriptor.get_num_qkv_args()]
