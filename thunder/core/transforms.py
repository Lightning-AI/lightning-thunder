...
augmented_forward_impls = {
    ...
    prims.PrimIDs.SIGN: lambda x: (prims.sign(x), (x,)),
...
backward_impls = {
    ...
    prims.PrimIDs.SIGN: lambda x, g: g * 0,
...
def remove_transient_outs_from_prologue(prologue_trc: Trace, computation_trc: Trace) -> Trace:
    ...
    excluded_proxies = {p.name for p in rets}

    new_prologue_trc = from_trace(prologue_trc)
    for bsym in prologue_trc.bound_symbols:
        if any([p.name in excluded_proxies for p in bsym.flat_proxy_outs]) or any(
            [p.name in excluded_proxies for p in bsym.flat_proxy_args]
        ):
            continue
