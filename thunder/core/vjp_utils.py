...
def get_saved_for_backward_tensors(trace: TraceCtx) -> tuple[TensorProxy]:
    ...
    connect_to_autograd_bsym = next(filter(lambda bsym: bsym.sym.id in (connect_to_autograd_impl.id, connect_to_torch_autograd.id), reversed(trace.bound_symbols)), None)
    utils.check(
        connect_to_autograd_bsym is not None,
        lambda: "Could not find the connect_to_autograd symbol in the trace",
    )
    saved_tensors = connect_to_autograd_bsym.kwargs["saved_tensors"]
    utils.check(
