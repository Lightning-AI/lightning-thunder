...
def get_saved_for_backward_tensors(trace: TraceCtx) -> tuple[TensorProxy]:
    ...
    utils.check(
        lambda: trace.fn.__name__ == "augmented_forward_fn",
        "The trace must be generated by Thunder's automatic differentiation",
    )
    ...
    utils.check(
        lambda: all(isinstance(t, TensorProxy) or t is None for t in saved_tensors),
        "All saved tensors must be TensorProxy or None",
    )
