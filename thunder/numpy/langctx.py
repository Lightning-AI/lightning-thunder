...
class NumPyCtx(LanguageContext):
    def __init__(self):
    ...
    def get_method(self, id: str, *args, **kwargs) -> Callable:
        ...
        if not primsctx.has_method(id):
            raise AttributeError(
                f"Attempting to call method {id} in the numpy language context, but it has no tensor inputs and the primitive language context (which handles numbers) doesn't have the method"
            )
...
numpyctx = NumPyCtx()
register_langctx(Languages.NUMPY, numpyctx)
register_langctx("numpy", numpyctx)
