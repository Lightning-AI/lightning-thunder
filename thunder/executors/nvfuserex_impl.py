...
from thunder.torch.experimental.dtensor_torch_and_prims import dtensor_mul_prim, dtensor_linear_prim
...
class FusionDefinitionWrapper:
    ...
    def __call__(self, *args):
        ...
        args = tuple(arg.wait() if isinstance(arg, torch.distributed._functional_collectives.AsyncCollectiveTensor) else arg for arg in args)
        ...
        if dist.is_available() and any(isinstance(t, torch.distributed.tensor.DTensor) for t in args):
            with annotate_for_profile(self.name):
                ...
                for out_tensor, out_sharding in zip(out_tensors, out_shardings):
                    mesh = dist.device_mesh.init_device_mesh("cuda", (out_sharding.mesh.size,), mesh_dim_names=args[0].device_mesh.mesh_dim_names)
