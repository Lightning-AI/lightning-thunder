...
class TransformerEngineTransformV2(Transform):
    ...
    def transform_trace_post_optimization(self, computation_trace, **kwargs):
        ...
        is_forward_trace = TraceTag.AUGMENTED_FORWARD in computation_trace.tags
        ...
        if is_forward_trace:
            ...
            unique_env = OrderedSet(Variable(x) for x in env)
...
#
def _te_activation_checkpointing_transform(fwd_trace: TraceCtx, bwd_trace: TraceCtx):
    """
    Looks at the backward trace and identifies which states need to be carried over from forward to backward trace in order to recompute the correct values.
    Also it removes the amax_updates for each op in the forward and keeps it only in the backward.

    Args:
        fwd_trace: Forward trace.
        bwd_trace: Backward trace.
    """
    # go through fwd_trace and build hash table
    fwd_linear_quantizers_map: dict[BoundSymbolRHS, list[AnyProxy]] = {}
    quantizer_state_map: dict[Variable, AnyProxy] = {}
    fwd_linear_state_map: dict[BoundSymbolRHS, AnyProxy] = {}

    # store the states and quantizers that need to be carried over in the backward

    for bsym in fwd_trace.bound_symbols:
        if "get_te_fp8_quantizers" in bsym.sym.name:
            q1 = variableify(bsym.output[0])
            quantizer_state_map[q1] = bsym.args[0]

        if bsym.sym.name == "te_functional_linear_fwd":
            pseudo_bsym = bsym.from_bsym(args=bsym.args[:-2])
            fwd_linear_quantizers_map[pseudo_bsym.rhs] = bsym.args[-2:]
            fwd_linear_state_map[pseudo_bsym.rhs] = quantizer_state_map[variableify(bsym.args[-2])]

    states_and_quantizers_to_carry_over = []
    # replace states with the ones from the forward
    ...
    for bsym in bwd_trace.bound_symbols:
        if "get_te_fp8_quantizers" in bsym.sym.name:
            q1 = variableify(bsym.output[0])
            quantizer_state_map[q1] = bsym.args[0]
        # add quantizers and states to the saved for backward
        if bsym.sym.name == "te_functional_linear_fwd":
            pseudo_bsym = bsym.from_bsym(args=bsym.args[:-2])
            ...
            # update the linear calls
            for src, dst in zip(bsym.args[-2:], quantizers):
                swap_map[variableify(src)] = dst

            # update the states in the amax updates after
            src_state = quantizer_state_map[variableify(bsym.args[-2])]
            swap_map[variableify(src_state)] = state

        new_bsym = bsym.from_bsym_swap_proxies(swap_map)
        new_bwd_boundsymbols.append(new_bsym)

    new_bwd_trace = from_trace(bwd_trace)
    new_bwd_trace.bound_symbols = new_bwd_boundsymbols

    new_fwd_boundsymbols = []
    swap_map: dict[Variable, Proxy] = {}
    # remove the amax updates for the checkpointed linears
    for bsym in fwd_trace.bound_symbols:
        if bsym.sym.name == "te_fp8_amax_and_scale_update":
            bsym_recipe_states = {variableify(i) for i in bsym.kwargs["states"]}

            if not (bsym_recipe_states - states_to_carry):
                for src, dst in zip(bsym.output, bsym.kwargs["tokens"]):
                    swap_map[variableify(src)] = dst
                continue

        new_bsym = bsym.from_bsym_swap_proxies(swap_map)
        new_fwd_boundsymbols.append(new_bsym)
    ...
    _, (saved_for_backward, env) = return_bsym.args
    unique_env = OrderedSet(Variable(x) for x in env)
    new_saved_for_backward = (
        *saved_for_backward,
        *states_and_quantizers_to_carry_over,
        *(unvariableify(x) for x in unique_env),
    )

    _update_forward_with_new_saved_for_backward(new_fwd_trace, new_saved_for_backward)
    _update_backward_with_new_saved_for_backward(new_bwd_trace, new_saved_for_backward)
