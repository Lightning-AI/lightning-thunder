...
analyze_single_pr(
    pr_number=456,
    gdrive_files=["SecurityGuidelines", "ThreatModel"]
)
```
...
for pr_num in [101, 102, 103, 104, 105]:
    result = analyze_single_pr(
        pr_number=pr_num,
        gdrive_files=ref_files
    )
...
2. **Fetches** the content (up to 10,000 characters per file)
3. **Adds** to the LLM prompt like this:
...
2. **Test with one PR:**
   ...
   analyze_single_pr(
       pr_number=123,
       gdrive_files=["ThunderBestPractices"]
   )
   ```

3. **Check the logs** (stderr) for:
   ```
...
4. **Review the analysis** - it should align with your best practices!
